...
impl GitHubClient {
	...
	/// - `repo` 仓库名称
	#[napi]
	pub async fn get_repo_info(&self, repo_owner: String, repo_name: String) -> Result<RepoInfo> {
		let client = create_client().await;
	...
	///
	#[napi]
	pub async fn get_user_repos(&self, options: Option<ReposListOptions>) -> Result<Vec<RepoInfo>> {
		let client = create_client().await;
