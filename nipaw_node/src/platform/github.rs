...
static GITHUB_CLIENT: LazyLock<RwLock<nipaw_github::GitHubClient>> = LazyLock::new(|| RwLock::new(nipaw_github::GitHubClient::default()));
...
impl GitHubClient {
	...
	#[napi]
	pub async fn get_repo_info(&self, repo_owner: String, repo_name: String) -> napi::Result<RepoInfo> {
		let client = GITHUB_CLIENT.read().await;
		let repo_info =
			client.get_repo_info((repo_owner.as_str(), repo_name.as_str())).await.map_err(|e| napi::Error::from_reason(format!("{:?}", e)))?;
		Ok(repo_info.into())
	}
